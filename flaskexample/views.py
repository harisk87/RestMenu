...
def mid_page():
    ...
    rest_name = restaurant_name_list[name_and_addr_idx]

    menu_item_list = all_menus[ all_menus.restaurant_name ==  rest_name].item_name.tolist()
    #menu_item_list = [s.decode('utf-8') for s in menu_item_list]
    return render_template('menu_item_render.html',menu_item_list=menu_item_list,rest_name=rest_name)
...
def result():
    ...
    idx = all_menus[(all_menus.restaurant_name == rest_name) & (all_menus.item_name == menu_item)].index[0]
    ...
    for i in top_num_sim_items_list:
        ...
        result_dict.append(dict(menu_item=menu_print, desc=desc_print, name=name_print, price=price_print,address = address_print, yelp_rating = yelp_rating_print) )
